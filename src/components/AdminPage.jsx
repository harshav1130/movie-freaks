...
const AdminPage = () => {
  ...
  // 👇 SAFE FETCH FUNCTION (Prevents White Screen)
  const fetchAll = async () => {
      try {
        const responses = await Promise.all([
            fetch(`${API_URL}/api/movies`).catch(() => ({ ok: false })),
            fetch(`${API_URL}/api/series`).catch(() => ({ ok: false })),
            fetch(`${API_URL}/api/anime`).catch(() => ({ ok: false }))
        ]);
        
        const m = responses[0].ok ? await responses[0].json() : [];
        const s = responses[1].ok ? await responses[1].json() : [];
        const a = responses[2].ok ? await responses[2].json() : [];
  ...
  const handleBannerSubmit = async (e) => {
      ...
      try {
        ...
        let method = 'POST';
        if (editingBanner) { url = `${API_URL}/api/admin/carousel/update/${editingBanner.id}`; method = 'PUT'; }
        const res = await fetch(url, { method: method, body: data });
        if(res.ok) { 
            alert("✅ Banner Saved!"); setBannerForm({ title: '', description: '', tag: 'Featured', videoUrl: '' });
            setBannerImageFile(null); setBannerVideoFile(null); setEditingBanner(null); setResetKey(Date.now()); fetchCarousel(); 
        } else alert("Failed");
  ...
  const cancelEditBanner = () => { setEditingBanner(null); setBannerForm({ title: '', description: '', tag: 'Featured', videoUrl: '' }); };
                            ...
                            <div key={item.id} style={{ background: '#1f1f1f', borderRadius: '8px', overflow: 'hidden', border: '1px solid #333', position: 'relative' }}>
                                ...
                                <div style={{ padding: '12px' }}>
                                    ...
                                    <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'10px'}}>
                                        ...
                                        <button onClick={() => handleDelete(item.id)} style={{background:'transparent', border:'none', color:'#e50914', cursor:'pointer', fontSize:'1.1rem'}}><FaTrash/></button>
                                    </div>
            ...
            {activeTab === 'add' && (
                <div style={formContainerStyle}>
                    ...
                    <form onSubmit={handleAddSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                        ...
                        <div style={{display:'flex', gap:'10px'}}>
                            <div style={{flex:1}}><label>Year</label><input type="text" value={addFormData.year} onChange={(e) => setAddFormData({...addFormData, year: e.target.value})} style={inputStyle} /></div>
                            <div style={{flex:2}}><label>Cast</label><input type="text" value={addFormData.cast} onChange={(e) => setAddFormData({...addFormData, cast: e.target.value})} style={inputStyle} /></div>
                        </div>
                        ...
                        <div style={{display:'flex', alignItems:'center', gap:'10px', background:'#333', padding:'10px', borderRadius:'4px'}}><input type="checkbox" checked={addFormData.featured} onChange={(e) => setAddFormData({...addFormData, featured: e.target.checked})} /><label>Add to Hero Carousel</label></div>
                        ...
                        <div style={fileBoxStyle}><label>Trailer</label><input key={resetKey} type="file" accept="video/mp4" onChange={(e) => setFiles({...files, trailer: e.target.files[0]})} style={{color:'#fff'}} /></div>
                        {addFormData.category === 'movie' && <div style={fileBoxStyle}><label>Movie File</label><input key={resetKey} type="file" accept="video/mp4" onChange={(e) => setFiles({...files, video: e.target.files[0]})} required style={{color:'#fff'}} /></div>}
